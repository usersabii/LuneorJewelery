...
/* ===== CART BADGE SYNC (header + bulle) ===== */
(function(){
  'use strict';

  function readCart(){
    // si tu as __SHOP.getCart, prends-le; sinon localStorage 'cart'
    if (window.__SHOP && typeof __SHOP.getCart === 'function') {
      try { return __SHOP.getCart() || []; } catch { return []; }
    }
    try { return JSON.parse(localStorage.getItem('cart')||'[]'); } catch { return []; }
  }
  function computeQty(list){ return list.reduce((s,i)=> s + Number(i.qty||0), 0); }

  window.updateCartBadges = function(totalQty){
    const n = String(totalQty);
    // badges header existants
    document.querySelectorAll('#js-cart-badge, .cart-badge, [data-cart-badge], #cartBadge, .nav-cart-badge')
  ...
  function refreshBadges(){ window.updateCartBadges(computeQty(readCart())); }

  // 1) au chargement
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', refreshBadges, {once:true});
  } else { refreshBadges(); }

  // 2) après chaque "ajout au panier" — on wrap __addOnce si présent
  if (typeof window.__addOnce === 'function') {
    const prev = window.__addOnce;
    window.__addOnce = function(ev){
      const out = prev.apply(this, arguments);
      // petit délai => le temps que ton code écrive localStorage
      setTimeout(refreshBadges, 60);
      return out;
    };
  } else {
    // si pas de __addOnce, on écoute le clic sur .btn-add et on recalcule après
    document.addEventListener('click', (e)=>{
      if (!e.target.closest('.js-add, .add-to-cart, .btn-add, [data-action="add-to-cart"]')) return;
      setTimeout(refreshBadges, 60);
    }, true);
  }

  // 3) quand on vide le panier / confirme commande, etc. → appelle refreshBadges()
  document.addEventListener('cart:changed', refreshBadges);
  // (si tu vides via __clearCart, ajoute simplement: document.dispatchEvent(new Event('cart:changed'));)
...
/* ============================================================
  ...
  /* ===== SEARCH SUGGEST FIX (body-anchored dropdown) ===== */
(function(){
  'use strict';
  ...
  const SOURCES = {
    popular: [
      {label:'Colliers minimalistes', q:'collier minimaliste', tag:'Tendance'},
      {label:'Boucles perles', q:'perle', tag:'Classique'},
      {label:'Parures soirée', q:'set soirée', tag:'Occasion'},
      {label:'Bagues ajustables', q:'bague ajustable', tag:'Pratique'},
    ],
    shortcuts: [
      {label:'Colliers', type:'necklace'},
      {label:'Boucles',  type:'earring'},
      {label:'Bracelets',type:'bracelet'},
      {label:'Parures',  type:'set'},
      {label:'Cheville', type:'anklet'},
    ]
  };

  const $  = (s, r=document)=>r.querySelector(s);
  const $$ = (s, r=document)=>Array.from(r.querySelectorAll(s));
  const GRID_ID='products-grid', SHOP_ID='shop-section';
  const REC_KEY='searchRecent'; const MAX_REC=6;

  function getRecent(){ try{ return JSON.parse(localStorage.getItem(REC_KEY)||'[]'); }catch{ return []; } }
  function addRecent(q){ const L=getRecent().filter(x=>x!==q); L.unshift(q); localStorage.setItem(REC_KEY, JSON.stringify(L.slice(0,MAX_REC))); }

  function showShop(){
    $$('.page-section[data-page="home"]').forEach(s=>s.classList.add('hidden'));
    $('#'+SHOP_ID)?.classList.remove('hidden');
    $('#'+SHOP_ID)?.scrollIntoView({behavior:'smooth', block:'start'});
  }
  function filterByQuery(q){
    const grid = $('#'+GRID_ID); if(!grid) return;
    const query = (q||'').trim().toLowerCase();
    let shown=0;
    $$('#'+GRID_ID+' .product-card').forEach(card=>{
      const col = card.closest('.col, [class*="col-"]') || card;
      const hay = [
        card.querySelector('.product-title')?.textContent || '',
        card.dataset.type||'', card.dataset.metal||'', card.dataset.desc||''
      ].join(' ').toLowerCase();
      const ok = query ? hay.includes(query) : true;
  ...
  // index produits (titres) pour suggestions
  function productIndex(){
    return $$('#'+GRID_ID+' .product-card').map(c=>({
      title:(c.querySelector('.product-title')?.textContent||'').trim()
    }));
  ...
  // --- dropdown ancré au <body> pour éviter tout souci de wrapper/z-index
  const DROP_ID = 'searchSuggestPortal';
  function ensureDrop(){
    let d = document.getElementById(DROP_ID);
    if(!d){
      ...
      d.style.maxWidth='480px';
      d.style.background='#fff';
      d.style.border='1px solid rgba(0,0,0,.08)';
      d.style.borderRadius='12px';
      d.style.boxShadow='0 16px 40px rgba(0,0,0,.14)';
  ...
  function placeDropUnderInput(input){
  ...
  function hideDrop(){ const d=$('#'+DROP_ID); if(d) d.style.display='none'; }

  function renderSuggestionsFor(input){
    const d = placeDropUnderInput(input);
    const q = (input.value||'').trim().toLowerCase();
    const idx = productIndex();
    const prod = q ? idx.filter(x=>x.title.toLowerCase().includes(q)).slice(0,5) : idx.slice(0,5);

    let html = '';
    // chips
    html += `<div class="search-suggest__section" style="padding:10px 12px;">
      <div class="search-chip-row" style="display:flex;flex-wrap:wrap;gap:8px;">
        ${SOURCES.shortcuts.map(s=>`<button class="search-chip" data-type="${s.type}">${s.label}</button>`).join('')}
      </div>
    </div>`;
    // populaire
    html += `<div class="search-suggest__title" style="margin:4px 12px 6px;color:#888;font-weight:700;font-size:12px;text-transform:uppercase;">Populaire</div>`;
    html += SOURCES.popular.map(p=>`
      <div class="search-item" data-q="${p.q}" style="display:flex;gap:10px;align-items:center;padding:10px 12px;cursor:pointer;">
        <span class="tag" style="font-size:12px;padding:2px 8px;border-radius:999px;background:#f1ede5;color:#5c4a3e;font-weight:600">${p.tag}</span>
        <div>
          <div>${p.label}</div>
          <div class="muted" style="font-size:12px;color:#777">${p.q}</div>
        </div>
      </div>`).join('');
    ...
    const rec = getRecent();
    if(rec.length){
      html += `<div class="search-suggest__title" style="margin:8px 12px 6px;color:#888;font-weight:700;font-size:12px;text-transform:uppercase;">Récemment</div>`;
      html += rec.map(r=>`<div class="search-item" data-q="${r}" style="padding:10px 12px;cursor:pointer;">${r}</div>`).join('');
    }

    // produits
    if(prod.length){
      html += `<div class="search-suggest__title" style="margin:8px 12px 6px;color:#888;font-weight:700;font-size:12px;text-transform:uppercase;">Produits</div>`;
      html += prod.map(p=>`<div class="search-item" data-q="${p.title}" style="padding:10px 12px;cursor:pointer;">${p.title}</div>`).join('');
    }

    d.innerHTML = html;

    // interactions
    d.querySelectorAll('[data-q]').forEach(el=>{
      el.onclick = ()=>{
        const val = el.getAttribute('data-q');
        input.value = val;
        addRecent(val);
        hideDrop();
        showShop(); filterByQuery(val);
      };
    });
  ...
  function bindSearchInput(input){
    if(!input) return;
    const update = ()=> renderSuggestionsFor(input);
    input.addEventListener('focus', update);
    input.addEventListener('input', update);
    input.addEventListener('keydown', (e)=>{
      if(e.key==='Enter'){
        const v=input.value.trim(); if(!v) return;
        addRecent(v); hideDrop(); showShop(); filterByQuery(v);
      }
      if(e.key==='Escape'){ hideDrop(); input.blur(); }
    });
    ...
    document.addEventListener('click', (e)=>{
      ...
      const d = $('#'+DROP_ID);
      if(d && !d.contains(e.target)) hideDrop();
    });
  }

  // brancher les 2 champs (sidebar + mobile header)
  bindSearchInput(document.querySelector('.shop-sidebar .filter-search input'));
  bindSearchInput(document.getElementById('mobileSearchInput'));
})();
